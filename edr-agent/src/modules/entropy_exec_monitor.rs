...
fn attach_by_section_or_fallback(bpf: &mut Bpf, prog_name: &str) -> anyhow::Result<()> {
    let prog = bpf
    ...
    if let Some(sec) = prog.section() {
        if let Some(rest) = sec.strip_prefix("tracepoint/") {
            ...
            if !cat.is_empty() && !ev.is_empty() {
                tp.attach(cat, ev)?;
                log(&format!("✔️ attached tracepoint {}/{} (from section)", cat, ev));
                return Ok(());
            }
        }
    }

    // Fallback mapping if section isn't informative
    ...
    tp.attach(cat, ev)?;
    log(&format!("✔️ attached fallback tracepoint {}/{} for {}", cat, ev, prog_name));
    Ok(())
}
...
pub fn start_entropy_exec_monitor() {
    ...
    thread::spawn(|| {
        ...
        for name in ["trace_execve", "trace_openat", "trace_mmap"] {
            if let Err(e) = attach_by_section_or_fallback(&mut bpf, name) {
                    ...
                    thread::spawn(move || {
                        ...
                        loop {
                            match buf.read_events(&mut slots) {
                                Ok(events) => {
                                    if events.lost > 0 {
                                        log(&format!("⚠️ Lost {} entropy events (CPU {})", events.lost, cpu_id));
                                    }

                                    // Only process the first `events.read` buffers
                                    for slice in &slots[..events.read] {
                                        ...
                                        let entropy_opt = (|| {
                                            ...
                                            Some(calculate_entropy(&contents))
                                        })();
                                        ...
                                        let severity = if entropy >= 7.9 { 0.95 } else { 0.6 };
