...
pub fn start_ebpf_ipc_abuse_watch() {
    ...
    thread::spawn(move || {
        let bpf = Bpf::load(include_bytes_aligned!("../ebpf/suspicious_ipc.bpf.o"));
                                ...
                                Ok(_) => {
                                    for buffer in &buffers {
                                        if let Some(evt) = parse_ipc_event(buffer) {
                                            ...
                                            map.insert("replay_tag".into(), "ipc_abuse_detected".into());
                                            map.insert("ipc_summary".into(), summary.clone());
                                            map.insert("timestamp".into(), ts.to_string());
                                            map.insert("soc_note".into(), "IPC abuse detected via syscall msgsnd".into());
                                            map.insert("gnn_escalate".into(), "true".into());
                                            map.insert("cpu".into(), format!("{:.2}", cpu));
                                            map.insert("mem_kb".into(), format!("{}", mem));
