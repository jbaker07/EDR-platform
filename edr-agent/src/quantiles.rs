...
impl Quantiles {
    ...
    pub fn new(compression: usize) -> Self {
        ...
        Self { cap, buf: Vec::with_capacity(cap), seen: 0, rng: XorShift64::new(seed) }
    }
    ...
    pub fn p50(&self) -> Option<f64> { self.p(0.50) }
    pub fn p75(&self) -> Option<f64> { self.p(0.75) }
    pub fn p90(&self) -> Option<f64> { self.p(0.90) }
    pub fn p95(&self) -> Option<f64> { self.p(0.95) }
    pub fn p99(&self) -> Option<f64> { self.p(0.99) }

    /// Number of finite samples currently in the reservoir.
    pub fn count(&self) -> usize { self.buf.iter().filter(|x| x.is_finite()).count() }
}
