...
fn discover_ringbuf_maps(obj: &libbpf_rs::Object) -> Vec<String> {
    ...
    if ordered.is_empty() {
        ...
        eprintln!(
            "[ebpf/syscalls] ringbuf maps available: {}",
            ordered.join(", ")
        );
    }
    ordered
...
pub fn start_syscall_reader(writer: Arc<Mutex<TelemetryWriter>>) {
    ...
    if force || super::pins_available(&prefix) {
        ...
        for name in [
            ...
            if Path::new(&path).exists() {
                log::info!("subscribe: syscall_reader source=ringbuf path={path}");
                let handler_pinned = handler.clone();
                if let Err(e) =
                    events_reader::start_ringbuf_reader_from_pinned(&path, move |data| {
                        handler_pinned.as_ref()(data);
                    })
                {
                    log::error!("pinned subscribe failed for syscall_reader: {e}");
                } else {
                    subscribed = true;
                }
            }
        ...
        if !subscribed {
            log::error!("pinned subscribe failed: missing syscall ringbuf pins under {base}");
    ...
    {
        log::warn!("loader: syscall_reader using .bpf.o (feature explicitly enabled)");
        ...
        let open = match ObjectBuilder::default().open_file(&obj_path) {
            Ok(o) => o,
            Err(e) => {
                eprintln!("[ebpf/syscalls] open {obj_path} failed: {e}");
                return;
            }
        };
        let mut obj = match open.load() {
            Ok(o) => o,
            Err(e) => {
                eprintln!("[ebpf/syscalls] load failed: {e}");
                return;
            }
        };
        ...
        attach_tp_if_present(
            &mut obj,
            &mut links,
            &["tp_sched_process_exec", "tp_sched_process_exe"],
            "sched",
            "sched_process_exec",
        );
