...
pub fn crawl_files() -> Vec<FingerprintEntry> {
    ...
    // Fix temporary value drop by binding to variables
    ...
    for dir in dirs {
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            let hash = match sha256_digest(path.to_string_lossy().as_ref()) {
                Some(h) => h,
                None => {
                    log::warn!("Skipping file due to failed hash: {:?}", path);
                    continue;
                }
            };
            ...
            entries.push(FingerprintEntry::File {
                ...
                entropy: Some(entropy), // fix type mismatch
...
pub fn crawl_network() -> Vec<FingerprintEntry> {
    ...
    let protocols = [("tcp", "-t"), ("udp", "-u")];
    let socket_cmds = ["ss", "netstat"]; // fallback options

    for (proto, flag) in protocols.iter() {
        let output = Command::new("ss")
            .arg(flag)
            .arg("-lpnH") // listening, show PID, no headers
            .output()
        ...
        if let Ok(output) = output {
            ...
            for line in content.lines() {
                let line = line.trim();
                if line.is_empty() || line.contains("State") {
                    continue;
                }

                // ss format: Proto Recv-Q Send-Q Local Address:Port Peer Address:Port
                let cols: Vec<&str> = line.split_whitespace().collect();
                if cols.len() < 5 {
                    continue;
                }
                ...
                let local = cols[3];
                let direction = if line.contains("LISTEN") || cols[0].starts_with("tcp") {
                    "inbound"
                } else {
                    "outbound"
                };
                let process_name = Some("bash".to_string());
                if let Some((ip, port)) = local.rsplit_once(':') {
                    if let Ok(port_num) = port.parse::<u16>() {
                        entries.push(FingerprintEntry::NetworkEndpoint {
                            ip: ip.to_string(),
                            port: port_num,
                            protocol: proto.to_string(),
                            process: Some(process_name.unwrap_or_else(|| "unknown".into())),
                            category: "network_endpoint".into(),
                            tags: vec!["network".into(), "baseline".into()],
                            source_module: "net_watch".into(),
                            created_at: Some(now),
                            description: Some("Known-good network endpoint".into()),
                        });
        ...
        for pid_entry in pids.flatten() {
            ...
            for line in reader.lines().flatten() {
                ...
                let (start, end) = match addresses.split_once('-') {
                    Some((s, e)) => (usize::from_str_radix(s, 16).unwrap_or(0), usize::from_str_radix(e, 16).unwrap_or(0)),
                    None => continue,
                };
                ...
                let entropy = if exec || write {
                    ...
                    if let Ok(mut mem_file) = fs::File::open(&mem_path) {
                        let mut buf = vec![0u8; size_bytes.min(4096)]; // read max 4KB
...
pub fn crawl_known_cron_shells() -> Vec<FingerprintEntry> {
    ...
    let command = "/usr/bin/backup.sh".to_string();
    let user = "root".to_string();
    let uid = 0;
    let home_dir = Some("/root".to_string());
    let frequency = Some("@daily".to_string());
    let exec_capable = true;

    // Parse from /etc/default/cron
    if let Ok(file) = File::open("/etc/default/cron") {
        for line in BufReader::new(file).lines().flatten() {
            if let Some(shell) = line.strip_prefix("SHELL=") {
                known_shells.push(shell.trim().to_string());
            }
        }
    }

    // Parse /etc/crontab for shell references
    if let Ok(file) = File::open("/etc/crontab") {
        for line in BufReader::new(file).lines().flatten() {
            if line.contains("/bin/") || line.contains("/usr/bin/") {
                for token in line.split_whitespace() {
                    if token.starts_with('/')
                        && (token.contains("sh") || token.contains("bash") || token.contains("zsh"))
                    {
                        known_shells.push(token.to_string());
                    }
                }
            }
        }
    ...
    let category = "cron_shell".to_string();
    let tags = vec!["scheduled_task".to_string()];
    let source_module = "job_sched_monitor".to_string();
    let description = Some("Known good cron job shell".to_string());
    ...
    for path in auth_bins {
        ...
        if pb.exists() {
            ...
            entries.push(FingerprintEntry::AuthChain {
                ...
                signer: None, // Optional: fill in later with signature check
                verified: true, // Default to true unless verification fails later
                trusted_uid: owner,
...
pub fn crawl_process_lineage_baseline() -> Vec<FingerprintEntry> {
    ...
    for entry in fs::read_dir(proc_root).unwrap_or_else(|_| fs::read_dir("/dev/null").unwrap()) {
        if let Ok(entry) = entry {
            ...
            if let Ok(pid) = pid_str.parse::<i32>() {
                ...
                let ancestry = trace_ancestry(pid); // assumes this is implemented
...
pub fn crawl_gpg_ssh_keys() -> Vec<FingerprintEntry> {
    ...
    // Collect all home directories from /etc/passwd
                            ...
                            if fname.starts_with("id_") && !fname.ends_with(".pub") {
                                if let Ok(metadata) = fs::metadata(&path) {
                                    ...
                                    if File::open(&path)
                                        ...
                                        .is_ok()
                                    {
                                        let hash = Some(format!("{:x}", Sha256::digest(&file_contents)));
                                        let entropy = Some(calculate_entropy(&file_contents));
                                        let exec_capable =
                                            Some(metadata.permissions().mode() & 0o111 != 0);

                                        entries.push(FingerprintEntry::GpgSshKey {
                                            path: path.to_string_lossy().to_string(),
                                            hash: hash.expect("Hash must exist when inserting fingerprint"),
                                            owner: metadata.uid(),
                                            trusted_uid: 0,
                                            permissions: Some(metadata.permissions().mode()),
                                            entropy,
                                            ...
                                            created_at: Some(current_timestamp()),
                                            description: Some("GPG/SSH key discovered during baseline".into()),
                                        });
                                    }
                                }
                            }
                                ...
                                if let Ok(metadata) = fs::metadata(&path) {
                                        ...
                                        .is_ok()
                                    {
                                        ...
                                        entries.push(FingerprintEntry::GpgSshKey {
                                            ...
                                            trusted_uid: 0,
                                            permissions: Some(metadata.permissions().mode()),
                                            entropy: entropy.map(|v| v as f64),
                                            exec_capable,
                                            category: "key".into(),
                                            tags: vec!["ssh_key".into()],
                                            source_module: "key_crawler".into(),
