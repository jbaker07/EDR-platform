...
| The ServerLevel handed in is the one whose weather, game rules (getGameRules() is declared on ServerLevel, not Level) and saved data a per-level feature should read. | `direct_reference` | `extracted/minecraft_members.json`; [[40-Interfaces/net.minecraft.server.level.ServerLevel|ServerLevel]] |
| This is the event the reference lantern uses; the earlier name END_WORLD_TICK does not exist in this Fabric API and fails to compile. | `direct_reference` | `capability/subscribe_event.fabric_server_tick` |
